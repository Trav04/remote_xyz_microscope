/**
 * **************************************************************
 * @file mylib/txradio.c
 * @author Travis Graham
 * @date 14/08/2024
 * @brief mylib driver file for the tx radio
 ***************************************************************
 * EXTERNAL FUNCTIONS
 ***************************************************************
...
#define MYCONFIG
    ...
    #define default_addr default_addr
#endif
#else
...
/*
Global packet to be transmitted.
Allows for a 16byte hamming encoded message.
*/
uint8_t packet[32];

/* Radio Packets Queue to be transmitted*/
QueueHandle_t RadioPacketsQueue;

/* txRadio Task Handl (For RCMEXT Commands) */
TaskHandle_t txRadioTaskHandle;
...
int nrf24l01plus_recv(uint8_t *rx_buf)
{
    ...
    { // Check if Data Ready RX FIFO interrupt triggered
        HAL_Delayus(10);
        ...
        NRF_CE_LOW();
        nrf24l01plus_wr(NRF24L01P_STATUS, 0x70); // clear RX_DR or TX_DS or MAX_RT interrupt flag. Writing 1 clears bit
        return 1;
    }
...
/**
 * @brief  NRF24l01Plus send packet Function. Writes buffer to txFIFO via SPI. 
 * Set's CE Register value to ensure transmit state is entered before 
 * transmission.Transmits data by delaying by appropriate amount and returns to 
 * Standby-I mode.
 * @param  None
 * @retval None
 */
...
void txradio_ctrl_tsk(void *pvParameters)
{
    ...
    // Write the default channel to the channel register
    nrf24l01plus_wr(NRF24L01P_RF_CH, MYRADIOCHAN);

    RadioPacketsQueue = xQueueCreate(10, sizeof(packet));
        int flag = 0;

    for(;;) {
        if (RadioPacketsQueue != NULL) {
            if (xQueueReceive(RadioPacketsQueue, &packet, 10)) {
                // Transmit packet payload
                nrf24l01plus_send(packet);
                #ifdef DEBUG
                    taskENTER_CRITICAL();
                    uint8_t status = nrf24l01plus_rr(0x07); //read the status register
...
extern void init_tsk_txradio(void)
{
